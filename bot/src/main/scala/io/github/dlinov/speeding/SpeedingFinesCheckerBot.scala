...
class SpeedingFinesCheckerBot(
    token: String,
    override val dao: Dao[IO], tessDataPath: String)(
    ...
    if (msgText.forall(!_.startsWith("/"))) {
      skipBots {
        val (updDataResp, maybeFinesResp) = (for {
          ...
          replies ← EitherT {
            dao
              .updateDriver(chatId, driverInfo).map(x => x)
        ...
        reply(updDataResp)
        maybeFinesResp.map(reply(_).map(_ => ())).getOrElse(IO.unit) // TODO: find better way to ignore reply return value
      }
    } else {
      IO.unit
    }
